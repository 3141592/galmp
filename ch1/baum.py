...
startpos_x = width / 2 - 200
startpos_y = height / 2 +300  
...
def drawTree(input, startpos_x, startpos_y):
    a = 0  # angle
    i = 0  # counter for processcalculation
    processOld = 0  # old process
    #newpos = oldpos
    oldpos = startpos_x, startpos_y
    newpos = startpos_x, startpos_y
    ...
    for character in input:  # process for drawing the l-system by writing the string to the screen
        ...
        if character == 'A':  # magic happens here
            newpos = polar_to_cart(a + angleoffset, step, startpos_x, startpos_y)
...
if __name__ == '__main__':
    ...
    print(tree)
    drawTree(tree, startpos_x, startpos_y)
    pygame.display.flip()
